<?php
/**
 * shCustomTitleTag support for com_realestatemanager component.
 *
 * @author      $Author:  $
 * @copyright   
 * @package    
 ...
 *
 * Some parts from:
 * 404SEFx support for com_realestatemanager component.
 *
 */
...
// ===========+++==+++===+++===+++===++===++===+++==+++++++==+++===+++==+++===+++==+++===+++===+++===+++===+++==+++===++===+++===++==++==
 function & getConfig_1() {

    static $_config = null;

    if (empty($_config)) {  // config not read yet
      $_config = new shSEFConfig_1();
    }
    return $_config;

  }
// ===========+++==+++===+++===+++===++===++===+++==+++++++==+++===+++==+++===+++==+++===+++===+++===+++===+++==+++===++===+++===++==++==
class Sh404sefClassPageinfo_1  {
...
// ===========+++==+++===+++===+++===++===++===+++==+++++++==+++===+++==+++===+++==+++===+++===+++===+++===+++==+++===++===+++===++==++==
 /**
   ...
   */
 function & getPageInfo_1() {

    static $_instance = null;

    if(is_null( $_instance)) {
      $_instance = new Sh404sefClassPageinfo_1();
    }
...
$shPageInfo = & getPageInfo_1();  // get page details gathered by system plugin
$sefConfig = &  getConfig_1();
...
$shLangIso = shLoadPluginLanguage( 'com_realestatemanager', $shLangIso, 'COM_SH404SEF_CREATE_NEW', JPATH_SITE.'/components/com_realestatemanager/meta_ext');
...
if(isset($id)) {
  $query = "SELECT h.htitle, h.description FROM #__rem_houses AS h
...
//To set Title
$shCustomTitleTag = mylenStr($sitename." | ".$rem.$tagtitle, 75);
//To set meta Description
$shCustomDescriptionTag = mylenStr($rem.$tagtitle." | ".$desctitle, 200);
//To set meta KeywordsTag
$shCustomKeywordsTag = mylenStr($rem.$tagtitle, 250);
